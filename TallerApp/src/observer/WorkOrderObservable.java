...
public class WorkOrderObservable extends Observable implements Runnable {
	...
	public WorkOrderObservable(List<Service> listService) {
		...
		while (!endListWorkOrderBool) {
			...
			if (listService != null) {
				...
				for (Service service : listService) {
					...
					// if((workOrder.getListServices().size() - cont) == 0)
					// {
					// WorkOrder workOrderUp = updateStateWorkOrder(workOrder);
					// managerWorkOrderService.updateStateWorkOrder(workOrderUp);
					// }
					// cont--;
					// }
					// if(cont == 0)
